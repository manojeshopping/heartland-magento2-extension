...
class Payment extends \Magento\Payment\Model\Method\Cc
{
    ...
    try {
        ...
        if (is_string($response)) {
            ...
            try {
                if( $response->tokenData->tokenValue){
                    // \HPS\Heartland\Model\StoredCard::setStoredCards
                    HPS_STORED_CARDS::setStoredCards($response->tokenData->tokenValue, $response->cardType, $CcL4, $this->getAdditionalData()['cc_exp_month'], $this->getAdditionalData()['cc_exp_year']);
                }else{
                    // \Psr\Log\LoggerInterface::error
                    $this->_logger->error(__('Payment MultiUse Token: ' . $response->tokenData->responseMessage));
                }/**/
            }catch (\Exception $e) {
                // \Psr\Log\LoggerInterface::error
    ...
    public function getToken(){
        if( preg_match('/^[\w]{11,253}$/', (string) $this->_token_value) !== 1){
            ...
            if (preg_match('/^[\d]$/', $data['token_value']) === 1){ // it could be an index for multiuse token so if its only integer we  perform lookup
                ...
                $this->_token_value = HPS_STORED_CARDS::getToken($data['token_value']);
            }else{
    ...
    public function saveMuToken(){
        $this->_save_token_value = (int) $this->getAdditionalData()['_save_token_value'];
    ...
    public function log($param, $txt = ''){
        return null; //$this->_logger->log(100,$txt . print_r($param,true));
    }
}